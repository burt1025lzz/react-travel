...
export const ProductIntro: React.FC<PropsType> = (props) => {
  ...
  return (
    <div className={styles.intro__container}>
      ...
      <div className={styles.intro__detail__content}>
        <Typography.Text style={{marginLeft: 20}}>
          ¥ <span className={styles.intro__detail__strong__text}>{price}</span>{" "}
          /人起
        </Typography.Text>
        <Typography.Text style={{marginLeft: 50}}>
          <span className={styles.intro__detail__strong__text}>{rating}</span>{" "}
          分
        </Typography.Text>
      </div>
      <Carousel autoplay slidesToShow={3}>
        {pictures.map((item, index) => (
          <Image key={index} height={150} src={item}/>
        ))}
      </Carousel>
      <Table<RowType>
        columns={columns}
